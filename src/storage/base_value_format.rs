...
impl InternalValue {
    ...
    pub fn set_relative_timestamp(&mut self, ttl: u64) -> Result<()> {
        let current_micros = Utc::now().timestamp_micros() as u64;
        self.etime = current_micros.checked_add(ttl).ok_or_else(|| {
            StorageError::InvalidFormat(
                "Timestamp overflow when calculating relative etime".to_string(),
            )
        })?;
        Ok(())
    }
...
pub struct ParsedInternalValue {
    pub value: BytesMut,
    pub data_type: DataType,
    /// When used to represent MetaValue, the 'user_value' field is decoded to len
...
impl ParsedInternalValue {
    pub fn new(
        ...
        Self {
            value: value,
            data_type: data_type,
            user_value_range: user_value_range,
